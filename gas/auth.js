...
function verifyAuth_(token) {
  try {
    ...
    if (!sheet) { console.log('認証シートなし'); return false; }
    ...
    console.log('トークン不一致: input=[' + input + ']');
    return false;
  } catch(e) { console.log('認証エラー: ' + e.message); return false; }
}
...
function setupAuthSheet_() {
  ...
  console.log('認証シート作成: user=' + user + ' token=' + token);
